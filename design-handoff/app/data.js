/* VibeMail Glass — sample mailbox (fake data, grounded in the backend Message model)
   Each record maps to the CONTRACT.md Message shape:
     id, threadId, from, to, subject, date, snippet, bodyPlain,
     labelIds[], isRead, isStarred, status, draftId
   Plus a few UI-display conveniences (senderName/senderEmail parsed from `from`,
   a short `time` label, and an expanded `thread[]` of rendered messages).        */

(function () {
  const me = "you@vibemail.app";

  // helper: build a thread message
  const t = (from, email, date, body) => ({ from, email, date, body });
  ...
  // "Personal" intentionally has no INBOX messages → exercises the empty-label state.
  const LABELS = ["Social", "Updates", "Forums", "Shopping", "Promotions"];

  window.VM_DATA = { MESSAGES, LABELS, ME: me };
})();
